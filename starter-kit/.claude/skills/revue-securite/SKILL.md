...
description: >
  ...
  modifie une route HTTP, la validation d'un champ, ou la configuration de
  démarrage de l'application.
when_to_use: >
  Déclencher aussi sur « c'est risqué ? », « avant que je pousse »,
  « quelqu'un peut exploiter ça ? », « est-ce que c'est safe ? »,
  « rien de dangereux ? »
...
<!--
GABARIT — atelier A2.
La `description` est l'interface de la skill : elle doit dire QUOI et QUAND.
Une description qui ne dit pas QUAND ne se déclenchera jamais automatiquement.

Attention à la différence, c'est la confusion n°1 :
...
3. Consulter `references/checklist.md` **uniquement** si le diff touche une route
   HTTP ou la configuration de démarrage.
...
```
app.py:41 — **majeur** — le paramètre `completed` est comparé sans être validé
contre une liste de valeurs autorisées — restreindre à {"true","false"} et
retourner 400 sinon.
```
...
- Ne jamais afficher la valeur d'un secret trouvé : indiquer le fichier et la ligne.
